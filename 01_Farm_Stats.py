...
from source.my_func import cal_PF, cal_RE24, my_round, cal_pa_count
...
with cols[2]:
    league_select = st.selectbox(
        "League",
        ["All Leagues", "イースタン", "ウエスタン"],
        index=0
    )
    
if league_select == "All Leagues":
    team_list = ["All Teams"] + el_list + wl_list
elif league_select == "イースタン":
    team_list = ["All Teams"] + el_list
elif league_select == "ウエスタン":
    team_list = ["All Teams"] + wl_list
with cols[3]:
    if group_index <= 1:
...
if stats_type == "Batting":
    split_list = ["No Splits", "Yesterday", "Last 7days", "Last 14days", "Last 30days", 
         "March/April", "May", "June", "July", "August", "Sept~", "vsRHP", "vsLHP",
         ...
         "vs 日本ハム", "vs 楽天", "vs DeNA", "vs 巨人", "vs ヤクルト", "vs ロッテ", "vs オイシックス", "vs 西武",
         "vs オリックス", "vs 阪神", "vs ソフトバンク", "vs 広島", "vs くふうハヤテ", "vs 中日",
         "0 Outs", "1 Out", "2 Outs", 
    ...
    split_list = ["No Splits", "Yesterday", "Last 7days", "Last 14days", "Last 30days", 
         "March/April", "May", "June", "July", "August", "Sept~", "vsRHH", "vsLHH",
...
tabs_list = ["Dashboard", "Standard", "Advanced", "Plate Discipline", "Pitch Type", "Pitch Value"]
...
pf = cal_PF(PA_df=PA_df, league_type="2軍")
...
count_str_list = cal_pa_count(data)
...
if stats_type == "Batting":
    ...
    player_pa['threshold_pa'] = player_pa['team_g'] * 2.7
    ...
    if game_type == "レギュラーシーズン":
        ...
        sb_df = sb_df[(sb_df["game_type"] == "イ・リーグ") | (sb_df["game_type"] == "ウ・リーグ") | (sb_df["game_type"] == "ファーム交流戦")]
    elif game_type == "交流戦":
        ...
        sb_df = sb_df[sb_df["game_type"] == "ファーム交流戦"]
        ...
        sb_df = sb_df[(sb_df["game_type"] == "イ・リーグ") | (sb_df["game_type"] == "ウ・リーグ")]
    ...
    try:
        kouryusen = lg_game.loc[lg_game['League'] == "ファーム交流戦", 'game_id'].iloc[0]
    except:
        kouryusen = 0
    ...
    player_ip_qua['threshold_ip'] = player_ip_qua['team_g'] * 0.8
    ...
    league_fip_data = league_fip_data[["fld_league", "cFIP", "lgRA"]]
    ...
    if split == "No Split":
        ...
        events_df = events_df[events_df["game_date"].dt.month <= 4]
        ...
        vs_team = split[3:]
        events_df = events_df[events_df["fld_team"] == vs_team]
        plate_df = plate_df[plate_df["fld_team"] == vs_team]
    ...
    df = df.reset_index(drop=True)
    with tab[0]:
        pitch_cols = c_list + ["G", "GS", "IP", "K/9", "BB/9", "HR/9", "BABIP", "LOB%", "RA", "FIP"]
    ...
    with tab[2]:
        pitch_cols = c_list + ["K/9", "BB/9", "K/BB", "HR/9", "K%", "BB%", "K-BB%", "AVG", "BABIP", "LOB%", "RA-", "FIP-", "RA", "FIP", "R-F"]
